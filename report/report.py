...
#NETTOYAGE DU JEU DE DONNÉES 
   ...
   # st.write(" - Is there any outliers in our dataset ?")
    ...
    st.write("In this section we focussed on leveraging techniques, in order to create new variables that were not in the training data. This helped us simplify and accelerate data transformations, while improving the model precision. The feature engineering was achieved in three steps:")
    ...
    st.write("We first carried out exploratory analyses, with the aim of getting to know our database, its flaws and the set of variables that were in it. We proceeded to the treatment of missing data, descriptive statistics, visualization of correlation coefficients and the choice of variables for the model.")
    st.write("After that we went on with the feature engineering, in which we did an evaluation of the remaining variables and handled outliers. Finally, we carried out the modeling. At the end of the modeling, it could be deduced that the Gradient Boost model performed best with respect to the R².")
